/** biome-ignore-all lint/a11y/noStaticElementInteractions: we need to use the div to handle the onClick event */
/** biome-ignore-all lint/a11y/useKeyWithClickEvents: we need to use the div to handle the onClick event */
...
export default function WorkspaceMenu({ files, onImport }: Props) {
  ...
  function handleFileSelect(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (typeof parsed !== 'object' || parsed.version !== 1 || !Array.isArray(parsed.files)) {
  ...
  const allChecked = importCandidates !== null && selected.size === importCandidates.length
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-[#3c3c3c]">
              <div>
                <h2 className="text-sm font-medium text-[#d4d4d4]">Importar workspace</h2>
                {importError ? null : (
            ...
            <div className="flex-1 overflow-y-auto">
              {importError ? (
                ...
                <div className="px-4 py-6 text-center">
                  <p className="text-xs text-[#858585]">Nenhum arquivo encontrado no workspace exportado.</p>
                </div>
              ) : (
                <>
                  {/* Select all toggle */}
                  <div className="flex items-center gap-2 px-4 py-2.5 border-b border-[#3c3c3c]">
                    ...
                    />
                    <label htmlFor="select-all" className="text-xs text-[#858585] cursor-pointer select-none">
                  ...
                  {importCandidates.map((f, i) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-[#d4d4d4] truncate font-mono">{f.name}</p>
                        <p className="text-[11px] text-[#4e4e4e] mt-0.5">
                          {f.content.split('\n').length} linha
                          {f.content.split('\n').length !== 1 ? 's' : ''} · {f.content.length} caractere
