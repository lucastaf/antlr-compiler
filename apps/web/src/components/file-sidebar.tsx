...
export default function FileSidebar({ files, activeId, onSelect, onCreate, onDelete, onRename }: Props) {
    ...
    <aside className="w-52 shrink-0 flex flex-col overflow-hidden border-r border-[#3c3c3c] bg-[#252526]">
      ...
      <div className="flex-1 overflow-y-auto py-1">
        {files.map((file) => {
          ...
          return (
            // biome-ignore lint/a11y/noStaticElementInteractions: we need to use the div to handle the onClick event
            // biome-ignore lint/a11y/useKeyWithClickEvents: <we need to use the div to handle the onClick event>
            <div
              key={file.id}
            ...
            >
              <FileCode size={14} className={isActive ? 'text-[#569cd6] shrink-0' : 'text-[#858585] shrink-0'} />
