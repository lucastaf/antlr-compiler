...
export class ExpressionTypeVisitor
  ...
  visitExpressao(ctx: ExpressaoContext): ASTExpressionNode {
    return this.visit(ctx.calculo_logico_ou()!)
  }
  ...
  private resizeArray<T>(array: Array<T>, expectedSize: number, defaultValue: T): Array<T> {
    return Array.from({ length: expectedSize }, (_, i) => (i < array.length ? array[i] : defaultValue))
  }
  ...
  private buildBinaryChain(
    ...
    for (let i = 0; i < operators.length; i++) {
      ...
      if (this.isUnknown(current.type) || this.isUnknown(rightNode.type)) {
        current = this.getMathOperation(current, operatorText, rightNode, ctx)
        continue
      }

      if (!validate(current.type, rightNode.type)) {
        this.addError(ctx, errorMessage, 'Error')
  ...
  visitValor_calculo(ctx: Valor_calculoContext): ASTExpressionNode {
    if (ctx.NUMERICO()) {
    ...
    if (ctx.VARIABLE()) {
      ...
      if (!symbol) {
        this.addError(ctx, `Variável '${name}' não declarada`, 'Error')
  ...
  visitFunction_call(ctx: Function_callContext) {
    ...
    if (symbol?.parametersCount !== (parameters?.length ?? 0)) {
      this.addError(
        ctx,
        `Número de parametros inválidos para ${functionName}, eram esperado ${symbol?.parametersCount} mas enviados ${parameters?.length ?? 0}`,
        'Warning',
      )
    }

    const parametersResized = this.resizeArray<ASTExpressionNode>(
      parameters ?? [],
      symbol.parametersCount,
      new NumberLiteral(0, ctx),
    )
  ...
  visitCalculo_parenteses(ctx: Calculo_parentesesContext): ASTExpressionNode {
    if (ctx.expressao()) {
  ...
  visitCalculo_unario(ctx: Calculo_unarioContext): ASTExpressionNode {
    if (ctx.calculo_unario()) {
      const innerNode = this.visit(ctx.calculo_unario()!)

      if (!this.isNumeric(innerNode.type)) {
  ...
  visitCalculo_prioridade_2(ctx: Calculo_prioridade_2Context): ASTExpressionNode {
  ...
  visitCalculo_prioridade_1(ctx: Calculo_prioridade_1Context): ASTExpressionNode {
    return this.buildBinaryChain(
      ctx,
  ...
  visitCalculo_deslocamento(ctx: Calculo_deslocamentoContext): ASTExpressionNode {
    return this.buildBinaryChain(
      ctx,
  ...
  visitCalculo_relacional(ctx: Calculo_relacionalContext): ASTExpressionNode {
    return this.buildBinaryChain(
      ctx,
  ...
  visitCalculo_igualdade(ctx: Calculo_igualdadeContext): ASTExpressionNode {
    return this.buildBinaryChain(
      ctx,
  ...
  visitCalculo_bitwise_e(ctx: Calculo_bitwise_eContext): ASTExpressionNode {
    return this.buildBinaryChain(
      ctx,
  ...
  visitCalculo_bitwise_xou(ctx: Calculo_bitwise_xouContext): ASTExpressionNode {
    return this.buildBinaryChain(
      ctx,
  ...
  visitCalculo_bitwise_ou(ctx: Calculo_bitwise_ouContext): ASTExpressionNode {
    return this.buildBinaryChain(
      ctx,
  ...
  visitCalculo_logico_e(ctx: Calculo_logico_eContext): ASTExpressionNode {
    return this.buildBinaryChain(
      ctx,
  ...
  visitCalculo_logico_ou(ctx: Calculo_logico_ouContext): ASTExpressionNode {
    return this.buildBinaryChain(
      ctx,
  ...
  visitArray_access(ctx: Array_accessContext) {
    ...
    if (symbol.type !== 'array') {
      this.addError(
        ctx,
        `Não é possível acessar os elementos de ${symbol.name} pois a variavel não é uma array`,
        'Error',
      )
      return new UnknownExpressionNode(ctx)
    }
  ...
  visitLista_expressoes(ctx: Lista_expressoesContext): ASTExpressionNode {
    return new UnknownExpressionNode(ctx)
  }
}
